...
const UserDrawer = ({ isOpen, onClose, username, token }: UserDrawerProps) => {
    ...
    const fetchUserFiles = async () => {
        ...
        try {
            ...
            const response = await fetch(import.meta.env.VITE_API_URL + '/me/downloads/', {
                headers: {
            ...
            const formatedData: ConvertedFile[] = data.map((item: { cnv_task_id: string; cnv_file: string; cnv_completed_at: string; cnv_status: string; cnv_download_link: string; }) => ({
    ...
    const getStatusBadge = (status: string) => {
        switch (status) {
            case 'completed':
                return <span className="px-2 py-0.5 bg-green-900 text-green-300 rounded-full text-xs">Completed</span>;
            case 'failed':
                return <span className="px-2 py-0.5 bg-red-900 text-red-300 rounded-full text-xs">Failed</span>;
            case 'processing':
                return <span className="px-2 py-0.5 bg-blue-900 text-blue-300 rounded-full text-xs">Processing</span>;
            case 'queued':
                return <span className="px-2 py-0.5 bg-gray-700 text-gray-300 rounded-full text-xs">Queued</span>;
            default:
                return <span className="px-2 py-0.5 bg-gray-700 text-gray-300 rounded-full text-xs">{status}</span>;
        }
    };
                ...
                <div className="p-6">
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-10 text-gray-400">
                            ...
                            <p>No files found</p>
                            <p className="text-sm mt-2">Files you convert while logged in will appear here</p>
                        </div>
                    ) : (
                        <div className="space-y-3">
                            {files.map((file, index) => (
                                ...
                                >
                                    <div className="p-3">
                                        <h3 className="font-medium text-white truncate" title={file.original_filename}>
