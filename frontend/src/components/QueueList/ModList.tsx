...
import { ModListProps } from "../../interfaces/QueueList";
        ...
        {mods.map((mod) => (
          ...
          >
            ...
            <div className="mt-2">
              ...
              {mod.status === "uploading" &&
                mod.uploadProgress !== undefined && (
                  <div className="space-y-1">
                    <div className="flex justify-between text-xs text-gray-300">
                      <span>{mod.uploadProgress.toFixed(1)}%</span>
                      <span>
                        {formatFileSize(
                          (mod.uploadProgress / 100) * mod.file.size
                        )}{" "}
                        / {formatFileSize(mod.file.size)}
                      </span>
                    </div>
                    ...
                    <div className="flex justify-between text-xs">
                      <span className="text-indigo-300">
                        ...
                        {uploadSpeeds[mod.id]
                          ? formatFileSize(uploadSpeeds[mod.id]) + "/s"
                          : "Calculating..."}
                      </span>
                      {mod.uploadProgress < 100 && (
                        <span className="text-gray-400">
                          {uploadSpeeds[mod.id] && mod.uploadProgress < 99
                            ? "ETA: " +
                              Math.ceil(
                                (mod.file.size * (100 - mod.uploadProgress)) /
                                  100 /
                                  uploadSpeeds[mod.id]
                              ) +
                              "s"
